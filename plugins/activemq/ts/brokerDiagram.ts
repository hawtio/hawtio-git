  ...
  _module.controller("ActiveMQ.BrokerDiagramController", ["$scope", "$compile", "$location", "localStorage", "jolokia", "workspace", ($scope, $compile, $location, localStorage, jolokia, workspace) => {
    ...
    function redrawLocalBroker() {
      ...
      if ($scope.viewSettings.broker) {
        jolokia.search("org.apache.activemq:type=Broker,brokerName=*", Core.onSuccess((response) => {
    ...
    function onContainerJolokia(containerJolokia, container, id) {
      if (containerJolokia) {
          ...
          var destination = getOrAddNode(typeName, destinationName, properties, () => {
            ...
            if (!brokerName) {
              containerJolokia.search("org.apache.activemq:destinationType=" + destinationTypeName
                + ",destinationName=" + destinationName +",*", Core.onSuccess((response) => {
        ...
        if (brokerId && $scope.viewSettings.network && $scope.viewSettings.broker) {
          containerJolokia.request({type: "read", mbean: "org.apache.activemq:connector=networkConnectors,*"}, Core.onSuccess((response) => {
        ...
        if ($scope.viewSettings.consumer) {
          containerJolokia.search("org.apache.activemq:endpoint=Consumer,*", Core.onSuccess((response) => {
            angular.forEach(response, (objectName) => {
        ...
        if ($scope.viewSettings.producer) {
          containerJolokia.search("org.apache.activemq:endpoint=Producer,*", Core.onSuccess((response) => {
            angular.forEach(response, (objectName) => {
              var details = Core.parseMBean(objectName);
              if (details) {
                var properties = details['attributes'];
        ...
        if ($scope.viewSettings.producer) {
          containerJolokia.request({type: "read", mbean: "org.apache.activemq:endpoint=dynamicProducer,*"}, Core.onSuccess((response) => {
